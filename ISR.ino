...
//if we are here we passed the debounce
 ...
 //we wait again but longer 
   consoleOut("button still pressed, so not accidentally");
 // first the led is flashed
  ...
  { 
     ...
     consoleOut("button still pressed");
    ...
    // wdt_disable(); //othersise it's going to reset
     ...
     { 
         ...
         if (val == 0) // still pressed
         { 
               consoleOut(F("button still pressed, open accesspoint"));
               actionFlag = 11;
               //attachInterrupt(digitalPinToInterrupt(knop), isr, FALLING);
               return;
          ...
          {
              consoleOut("button finally released, reboot");
              ESP.restart(); // reboot
          }
  } 
 else 
  ...
  // if the switch is off it should go on or reversed
    //ledState += 1; // ledState = always 1 or greather, after this addition
      if (lampState == 1) // lampState was 1 so switched on
      { 
